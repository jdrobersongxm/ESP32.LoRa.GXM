...
namespace devMobile.IoT.Rfm9x
{
    public sealed class Rfm9XDevice
    {
        ...
        public GpioController gpioController = new GpioController();
        public GpioPin led = null;
        ...
        [Flags]
        private enum RegIrqFlagsMask : byte
        {
        ...
        [Flags]
        private enum RegHopChannelFlags : byte
        {
            PllTimeout = 0b10000000,
            CrcOnPayload = 0b01000000,
        }

        private enum RegHopChannelMask : byte
        {
            PllTimeout = 0b10000000,
            CrcOnPayload = 0b01000000,
        ...
        public Rfm9XDevice(string spiPortName, int chipSelectPin, int resetPinNumber, int interruptPinNumber)
        {
            ...
            // Setup the reset pin
            ResetGpioPin = gpioController.OpenPin(resetPinNumber, PinMode.Output);
            ResetGpioPin.Write(PinValue.High);

            // Interrupt pin for RX message, TX done etc. notifications
            InterruptGpioPin = gpioController.OpenPin(interruptPinNumber, PinMode.Input);
